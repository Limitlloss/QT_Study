...
int main(int argc, char *argv[]) {
    ...
    QApplication a(argc, argv);
    {
        ...
        auto pal = a.palette();
        // for(int rr=0; rr<22; rr++) for(int gg=0; gg<3; gg++) {
        //     auto brush = pal.brush((QPalette::ColorGroup) gg, (QPalette::ColorRole) rr);
        //     auto cr = brush.color();
        //     qDebug().nospace()<<"pal.setBrush("<<(QPalette::ColorGroup) gg<<", "<<(QPalette::ColorRole) rr<<", QBrush({"<<cr.red()<<","<<cr.green()<<","<<cr.blue()<<","<<cr.alpha()<<"}, "<<brush.style()<<"));";
        // }
