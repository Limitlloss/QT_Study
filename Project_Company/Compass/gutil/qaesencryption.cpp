...
// invMixColumns函数将状态矩阵中的列进行逆混淆。
// 使用的乘法方法可能对不熟悉的人来说难以理解。
...
#endif
    //false or true这里非常重要
    //如果使用aes-ni，CBC和ECB模式的加密和解密扩展密钥不同。
    //但如果不用aes-ni，则加密和解密的扩展密钥相同。
